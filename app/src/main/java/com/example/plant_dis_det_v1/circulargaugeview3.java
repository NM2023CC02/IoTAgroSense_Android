...
public class circulargaugeview3 extends View {
    private final Paint borderPaint;
    private final Paint gaugePaint, textPaint;
    private float progress;

    public circulargaugeview3(Context context, AttributeSet attrs) {
        super(context, attrs);
        ...
        gaugePaint.setColor(Color.BLUE);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (progress > 0f) {
            ...
            canvas.drawText(text, textX - 60, textY + 50, textPaint);
        } else {
            // Draw the border of the gauge
            canvas.drawCircle(cx, cy, radius, borderPaint);
        }
    ...
    public void setProgress(float progress) {
        ...
        this.progress = Math.max(0f, Math.min(progress, 100f));
        //
    }
}
